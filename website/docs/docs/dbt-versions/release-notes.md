...
- **Enhancement**: The dbt Semantic Layer now fully supports the [`--favor-state` flag](/docs/cloud/about-cloud-develop-defer) when used with `--defer`. This enhancement allows you to always resolve `{{ ref() }}` functions using staging or production metadata, ignoring any development version.
